...
{
	http_event_t *event = (http_event_t *)e;
	http_interceptor_t *interceptor = (http_interceptor_t*)event->context;
...
void _http_client_create(void)
{
	http_client_t *c = NULL;
	int len = sizeof(http_client_t);

	c = (http_client_t*)platform_memory_alloc(len);
	...
	if (NULL == c->interceptor) {
		...
		c->interceptor = (http_interceptor_t*)platform_memory_alloc(len);
...
int _http_client_handle(http_client_t *c, const char *url, void *data, http_request_method_t method, http_event_type_t et, std::vector<std::string> headers, uint32_t flags, Response &response, size_t timeOut)
{
	if (NULL == c)
		c = http_client_lease();
...
{
	_http_client_handle(NULL, url, NULL, HTTP_REQUEST_METHOD_GET, http_event_type_on_body, headers, 0, response, timeOut);
	RETURN_ERROR(HTTP_SUCCESS_ERROR);
}
...
{
	_http_client_handle(NULL, url, (void*)data, HTTP_REQUEST_METHOD_POST, http_event_type_on_body, headers, 0, response, timeOut);
	RETURN_ERROR(HTTP_SUCCESS_ERROR);
}
...
{
	if (NULL == c) {
		c = _http_client_handle(c, url, NULL, HTTP_REQUEST_METHOD_GET, (http_event_type)(http_event_type_on_body | http_event_type_on_request), cb, http_keep_alive_flag);
	}
	else {
