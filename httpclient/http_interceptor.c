...
{
	http_interceptor_t *interceptor = (http_interceptor_t*)parser->data;
...
static int _http_on_headers_complete(http_parser *parser)
{
	...
	http_event_dispatch(interceptor->event, http_event_type_on_headers, interceptor, NULL, 0);
...
static int _http_on_message_begin(http_parser *parser)
{
	...
	http_event_dispatch(interceptor->event, http_event_type_on_response, interceptor, NULL, 0);
	return 0;
}
...
static int _http_on_body(http_parser *parser, const char *at, size_t length)
{
	...
	return http_event_dispatch(interceptor->event, http_event_type_on_body, interceptor, (void *)at, length);
}
...
static void _http_interceptor_parser_setting(http_interceptor_t *interceptor)
{
...
static int _http_interceptor_prepare(http_interceptor_t *interceptor)
{
	...
	if (NULL == interceptor->parser)
		interceptor->parser = (http_parser*)platform_memory_alloc(sizeof(struct http_parser));

	if (NULL == interceptor->parser_settings)
		interceptor->parser_settings = (http_parser_settings*)platform_memory_alloc(sizeof(struct http_parser_settings));
...
int http_interceptor_init(http_interceptor_t *interceptor, size_t timeOut)
{
	int res = HTTP_SUCCESS_ERROR;
	HTTP_ROBUSTNESS_CHECK(interceptor, HTTP_NULL_VALUE_ERROR);
	...
	interceptor->cmd_timeout = timeOut;
	interceptor->buffer_len = HTTP_DEFAULT_BUF_SIZE;
	interceptor->data_process = 0;
	interceptor->flag.all_flag = 0;
	interceptor->owner = NULL;

	if (NULL == interceptor->event)
		interceptor->event = http_event_init();

	if (NULL == interceptor->buffer)
		interceptor->buffer = (char*)platform_memory_alloc(interceptor->buffer_len);

	HTTP_ROBUSTNESS_CHECK((interceptor->event && interceptor->buffer), HTTP_MEM_NOT_ENOUGH_ERROR);
...
void http_interceptor_event_register(http_interceptor_t *interceptor, http_event_cb_t cb)
{
	...
	if (NULL != cb)
	{
		http_event_register(interceptor->event, cb);
	}
}
...
int http_interceptor_request(http_interceptor_t *interceptor, http_request_method_t method, const char *post_buf, std::vector<std::string>headers)
{
	int res = HTTP_SUCCESS_ERROR;
	HTTP_ROBUSTNESS_CHECK((interceptor && method), HTTP_NULL_VALUE_ERROR);
	...
	http_request_set_method(&interceptor->request, method);

	http_event_dispatch(interceptor->event, http_event_type_on_request, interceptor, NULL, 0);
	...
	{
		http_request_add_header(&interceptor->request, header.c_str());
	}
...
int http_interceptor_release(http_interceptor_t *interceptor)
{
	...
	if (interceptor->event) {
		http_event_release(interceptor->event);
		interceptor->event = NULL;
	}
	...
	size_t timeOut)
{
	...
	if (interceptor->flag.flag_t.again) {
		...
		interceptor->status = (http_interceptor_status)0;
	}
	...
	case http_interceptor_status_invalid:
		http_interceptor_init(interceptor, timeOut);
	...
	case http_interceptor_status_init:
		...
		connect_res = http_interceptor_connect(interceptor);
		if (connect_res != 0)
		{
			return connect_res;
		}
	...
	case http_interceptor_status_connect:
		http_interceptor_request(interceptor, mothod, (const char*)post_buf, headers);
	...
	case http_interceptor_status_response_complete:
		...
		response.code = interceptor->response.status;
		response.body.insert(response.body.end(), interceptor->buffer, interceptor->buffer + interceptor->buffer_len);
